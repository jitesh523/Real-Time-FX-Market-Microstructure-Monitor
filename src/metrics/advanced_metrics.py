...
from src.models import TickData, Trade
...
class KylesLambda:
    ...
    def get_metrics(self) -> dict:
        ...
        return {
            ...
            "avg_price_change": (
                np.mean(self.price_changes) if self.price_changes else None
            ),
            "avg_signed_volume": (
                np.mean(self.signed_volumes) if self.signed_volumes else None
            ),
        }
...
class AdvancedMetricsCalculator:
    ...
    def process_tick(self, tick: TickData):
        ...
        self.amihud_illiquidity.add_data_point(
            tick.timestamp, tick.mid_price, total_volume
        )
    ...
    def _calculate_liquidity_score(
        self, lambda_val: Optional[float], illiquidity: Optional[float]
    ) -> Optional[float]:
        """
