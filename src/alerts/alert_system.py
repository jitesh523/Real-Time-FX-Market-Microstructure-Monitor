...
class AlertManager:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "recent_critical": len(
                [a for a in self.alerts[-100:] if a.severity == AlertSeverity.CRITICAL]
            ),
        }
            ...
            with open(filepath, "w", newline="") as f:
                if self.alerts:
                    writer = csv.DictWriter(
                        f, fieldnames=self.alerts[0].to_dict().keys()
                    )
                    writer.writeheader()
...
class AlertRuleEngine:
    ...
    def check_anomaly(
        self, symbol: str, anomaly_type: str, anomaly_score: float
    ) -> Optional[Alert]:
        """Check for general anomaly."""
        severity = (
            AlertSeverity.CRITICAL if anomaly_score > 0.9 else AlertSeverity.WARNING
        )
