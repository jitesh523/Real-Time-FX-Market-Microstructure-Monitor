...
def render_depth_heatmap(orderbook: Dict, symbol: str):
    ...
    # Create DataFrame
    df = pd.DataFrame({"Price": all_prices, "Size": all_sizes, "Side": all_sides})
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            name="Bids",
            marker=dict(color=bid_sizes, colorscale="Greens", showscale=False),
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            name="Asks",
            marker=dict(color=ask_sizes, colorscale="Reds", showscale=False),
            hovertemplate="Price: %{y:.5f}<br>Size: %{x:.2f}<extra></extra>",
        )
    )
