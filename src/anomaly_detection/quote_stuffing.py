...
from src.models import OrderBook, TickData
...
class QuoteStuffingDetector:
    ...
    def __init__(self, window_seconds: int = 1, threshold: int = 100):
        ...
        logger.info(
            f"Initialized quote stuffing detector with threshold={threshold} updates/{window_seconds}s"
        )
    ...
    def detect_stuffing(self) -> Dict:
        ...
        if is_stuffing:
            ...
            logger.warning(
                f"Quote stuffing detected: {quote_rate} quotes, {orderbook_rate} OB updates in {self.window_seconds}s"
            )
...
class AdaptiveQuoteStuffingDetector:
    ...
    def detect_stuffing(self) -> Dict:
        ...
        if is_stuffing:
            self.stuffing_events += 1
            logger.warning(
                f"Adaptive quote stuffing detected: {quote_rate} quotes (threshold: {self.threshold:.1f})"
            )
