...
class IsolationForestDetector:
    ...
    def _tick_to_features(self, tick: TickData) -> np.ndarray:
        ...
        return np.array(
            [
                ...
                / (tick.bid_size + tick.ask_size + 1e-10),  # imbalance
            ]
        )
    ...
    def _retrain(self):
        ...
        self.is_fitted = True
        logger.debug(
            f"Retrained Isolation Forest with {len(self.feature_buffer)} samples"
        )
    ...
    def predict_tick(self, tick: TickData) -> Dict:
        ...
        if len(self.recent_anomaly_rate) >= 100:
            ...
            if abs(current_rate - self.contamination) > 0.05:
                ...
                logger.info(
                    f"Adjusting contamination: {self.contamination:.3f} -> {new_contamination:.3f}"
                )
