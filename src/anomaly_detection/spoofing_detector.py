...
class SpoofingDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized spoofing detector with depth_threshold={depth_threshold}"
        )
    ...
    def _cleanup_old_events(self, current_time: datetime):
        ...
        while (
            self.large_order_events
            and self.large_order_events[0]["timestamp"] < cutoff_time
        ):
    ...
    def detect_spoofing(self) -> Dict:
        ...
        if len(self.orderbook_history) < 2:
            return {"is_spoofing": False, "large_orders_detected": 0}
        ...
        is_spoofing = (
            len(cancellations) > 0 or imbalance_spoofing or total_large_orders > 3
        )

        if is_spoofing:
            self.spoofing_events += 1
            logger.warning(
                f"Spoofing detected: {total_large_orders} large orders, {len(cancellations)} cancellations"
            )
                    ...
                    cancellations.append(
                        {
                            ...
                            "order": order,
                            "time_to_cancel": (
                                current_ob.timestamp - event["timestamp"]
                            ).total_seconds(),
                        }
                    )
    ...
    def _is_order_still_present(self, order: Dict, orderbook: OrderBook) -> bool:
        ...
        size_match = (
            abs(level.size - order["size"]) / order["size"] < 0.2
        )  # 20% tolerance
