...
from src.models import MarketMetrics, TickData
...
class ZScoreDetector:
    ...
    def __init__(self, window_size: int = 100, threshold: float = 3.0):
        ...
        logger.info(
            f"Initialized Z-score detector with window={window_size}, threshold={threshold}"
        )
...
class MultiVariateZScoreDetector:
    ...
    def detect_tick_anomaly(self, tick: TickData) -> Dict:
        ...
        size_imbalance = (tick.bid_size - tick.ask_size) / (
            tick.bid_size + tick.ask_size + 1e-10
        )
        imbalance_result = self.imbalance_detector.detect_and_update(size_imbalance)

        # Overall anomaly if any metric is anomalous
        is_anomaly = (
            spread_result["is_anomaly"]
            or depth_result["is_anomaly"]
            or imbalance_result["is_anomaly"]
        )
    ...
    def detect_metrics_anomaly(self, metrics: MarketMetrics) -> Dict:
        ...
        # Detect imbalance anomaly
        imbalance_result = self.imbalance_detector.detect_and_update(
        ...
        if metrics.volatility is not None:
            volatility_result = self.volatility_detector.detect_and_update(
                metrics.volatility
            )
        ...
        is_anomaly = (
            ...
            or depth_result["is_anomaly"]
            or imbalance_result["is_anomaly"]
            or (volatility_result and volatility_result["is_anomaly"])
        )
